  ...
  return (
    <div className="animate-fade-in space-y-10">
      ...
      <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
        <div>
          <h2 className="text-3xl font-black text-slate-900 tracking-tight">Painel Diretor Studio</h2>
          <p className="text-slate-500 font-medium">Gestão centralizada de frequências e matrículas.</p>
        </div>
            ...
            {classes.map(cls => (
              <button key={cls.id} onClick={() => onSelectClass(cls.id)} className="bg-white p-8 rounded-[2rem] border border-slate-100 text-left hover:shadow-xl transition-all group">
                <div className="flex justify-between items-start mb-6">
                   <div className="text-[10px] bg-indigo-50 text-indigo-600 px-3 py-1 rounded-full font-black uppercase">{teachers.find(t => t.id === cls.teacherId)?.name}</div>
      ...
      {activeTab === 'reports' && (
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
          <div className="p-10 border-b border-slate-50 bg-slate-50/50 flex flex-col md:flex-row justify-between gap-6">
            <h3 className="text-2xl font-black text-slate-900">Relatórios de Frequência</h3>
      ...
      {activeTab === 'management' && (
        <div className="space-y-10">
          <div className="bg-indigo-600 p-10 rounded-[2.5rem] shadow-xl shadow-indigo-100 text-white">
             ...
             <form onSubmit={e => {
               ...
               e.currentTarget.reset();
                ...
                <select name="classId" required className="p-4 bg-white rounded-2xl text-slate-900 font-bold outline-none">
                   ...
                   {classes.map(c => <option key={c.id} value={c.id}>{c.name} ({teachers.find(t => t.id === c.teacherId)?.name})</option>)}
                </select>
